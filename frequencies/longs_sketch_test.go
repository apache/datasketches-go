...
func TestFrequentItemsStringSerial(t *testing.T) {
	...
	assert.NoError(t, err)
	err = sketch.UpdateMany(10, 100)
	assert.NoError(t, err)
	err = sketch.UpdateMany(10, 100)
	assert.NoError(t, err)
	err = sketch.UpdateMany(15, 3443)
	assert.NoError(t, err)
	err = sketch.UpdateMany(1000001, 1010230)
	assert.NoError(t, err)
	err = sketch.UpdateMany(1000002, 1010230)
	assert.NoError(t, err)
	...
	assert.NoError(t, sketch2.UpdateMany(190, 12902390))
	assert.NoError(t, sketch2.UpdateMany(191, 12902390))
	assert.NoError(t, sketch2.UpdateMany(192, 12902390))
	assert.NoError(t, sketch2.UpdateMany(193, 12902390))
	assert.NoError(t, sketch2.UpdateMany(194, 12902390))
	assert.NoError(t, sketch2.UpdateMany(195, 12902390))
	assert.NoError(t, sketch2.UpdateMany(196, 12902390))
	assert.NoError(t, sketch2.UpdateMany(197, 12902390))
	assert.NoError(t, sketch2.UpdateMany(198, 12902390))
	assert.NoError(t, sketch2.UpdateMany(199, 12902390))
	assert.NoError(t, sketch2.UpdateMany(200, 12902390))
	assert.NoError(t, sketch2.UpdateMany(201, 12902390))
	assert.NoError(t, sketch2.UpdateMany(202, 12902390))
	assert.NoError(t, sketch2.UpdateMany(203, 12902390))
	assert.NoError(t, sketch2.UpdateMany(204, 12902390))
	assert.NoError(t, sketch2.UpdateMany(205, 12902390))
	assert.NoError(t, sketch2.UpdateMany(206, 12902390))
	assert.NoError(t, sketch2.UpdateMany(207, 12902390))
	assert.NoError(t, sketch2.UpdateMany(208, 12902390))
...
func TestFrequentItemsByteResetAndEmptySerial(t *testing.T) {
	sketch, err := NewLongsSketchWithMaxMapSize(16)
	assert.NoError(t, err)
	assert.NoError(t, sketch.UpdateMany(10, 100))
	assert.NoError(t, sketch.UpdateMany(10, 100))
	assert.NoError(t, sketch.UpdateMany(15, 3443))
	assert.NoError(t, sketch.UpdateMany(1000001, 1010230))
	assert.NoError(t, sketch.UpdateMany(1000002, 1010230))
	sketch.Reset()
...
func TestFreqLongSliceSerDe(t *testing.T) {
	...
	assert.NoError(t, err)
	err = sk1.UpdateMany(10, 100)
	assert.NoError(t, err)
	err = sk1.UpdateMany(10, 100)
	assert.NoError(t, err)
	err = sk1.UpdateMany(15, 3443)
	assert.NoError(t, err)
	err = sk1.UpdateMany(1000001, 1010230)
	assert.NoError(t, err)
	err = sk1.UpdateMany(1000002, 1010230)
	assert.NoError(t, err)
...
func TestFreqLongStringSerDe(t *testing.T) {
	...
	sk1, err := NewLongsSketchWithMaxMapSize(minSize)
	assert.NoError(t, err)
	assert.NoError(t, sk1.UpdateMany(10, 100))
	assert.NoError(t, sk1.UpdateMany(10, 100))
	assert.NoError(t, sk1.UpdateMany(15, 3443))
	assert.NoError(t, sk1.UpdateMany(1000001, 1010230))
	assert.NoError(t, sk1.UpdateMany(1000002, 1010230))
...
func TestFreqLongs(t *testing.T) {
	...
	for h := 0; h < numSketches; h++ {
		...
		_, err = sketches[h].GetFrequentItems(ErrorTypeEnum.NoFalseNegatives)
		assert.NoError(t, err)
	}
}
...
func printSketch(t *testing.T, size int, items []int64) {
	...
	for i := 0; i < len(items); i++ {
		err := fls.UpdateMany(int64(i+1), items[i])
		assert.NoError(t, err)
	}
	fmt.Fprintf(&sb, "Sketch Size: %d\n", size)
...
func BenchmarkLongSketch(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		assert.NoError(b, sketch.Update(int64(i)))
	}
}
